const getRuleMetadata = require('../get-rule-metadata')
...
function toDefinitionObject(dfnString) {
...
function createDefinitionPair(dfnString) {
	const dfnLines = dfnString.split('\n')
	const term = dfnLines.shift()
	const definitions = []
	let definition
...
describe('taskforce-markdown', () => {
	const atomicRuleFrontmatter = yaml.safeLoad(outdent`
    id: bc4a75
  ...
  `)
	...
	const compositeRuleFrontmatter = yaml.safeLoad(outdent`
    id: c3232f
  ...
  `)
	...
	describe('get-rule-metadata', () => {
		it('returns the "Rule ID"', () => {
			...
			expect(dfnObject).toHaveProperty('Rule ID:', [`:   ${atomicRuleFrontmatter.id}`])
		})

		it('returns the "Last Modified" as "TODO"', () => {
			const metadata = getRuleMetadata({ frontmatter: atomicRuleFrontmatter })
			const dfnObject = toDefinitionObject(metadata)
			expect(dfnObject).toHaveProperty('Last Modified:', [`:   TODO (format Sep 25, 2019)`])
		})
		...
		describe('Rule Type', () => {
			...
			it('works for composite rules', () => {
				const metadata = getRuleMetadata({ frontmatter: compositeRuleFrontmatter })
				const dfnObject = toDefinitionObject(metadata)
		...
		describe('input aspects', () => {
			...
			it('returns text for each unknown input aspects', () => {
				...
				expect(dfnObject).toHaveProperty('Input Aspects:', [':   foo', ':   bar', ':   baz'])
			})
